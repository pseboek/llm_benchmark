...
def parse_args():
    ...
    parser.add_argument("--results-dir", default=str(ROOT / "data" / "benchmarks"), help="Directory for benchmark CSV results")
...
def main():
    ...
    if args.run_benchmark:
        results = run_benchmark(models=args.models, contexts=args.contexts, timeout_seconds=args.request_timeout, results_dir=args.results_dir)
        if results:
    ...
    if args.run_plan:
        ...
        try:
            results = run_benchmark_plan(plan, timeout_seconds=args.request_timeout, results_dir=args.results_dir)
        except Exception:
    ...
    if args.discover:
        ...
        report = build_report(
            ...
            benchmark_tasks=ModelScoutDB(args.db).list_benchmark_tasks(),
        )
        print(report)
        return
    ...
    if args.report:
        ...
        report = build_report(
            ...
            benchmark_tasks=db.list_benchmark_tasks(),
        )
